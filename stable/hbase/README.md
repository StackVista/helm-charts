...
Current chart version is `0.1.107`
...
| hdfs.image.tag | string | `"2.9.2-java11-4"` | Default container image tag for HDFS datanode. |
...
| stackgraph.image.tag | string | `"4.2.14"` | The default tag used for all omponents of hbase that are stackgraph version dependent; invividual service `tag`s can be overriden (see below). |
