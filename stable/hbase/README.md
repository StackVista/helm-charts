...
Current chart version is `0.1.128`
...
| hdfs.image.tag | string | `"2.10.1-java11-5"` | Default container image tag for HDFS datanode. |
...
| stackgraph.image.tag | string | `"4.6.6"` | The default tag used for all omponents of hbase that are stackgraph version dependent; invividual service `tag`s can be overriden (see below). |
