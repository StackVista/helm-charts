...
| agent.image.tag | string | `"2.0.6"` | Default container image tag. |
...
| agent.processAgent.blacklistPatterns | string | `"^s6-,^/sbin/,^/usr/sbin,^/pause,^/usr/bin/dbus-daemon,^-bash,^su$,^/bin/bash,/lib/systemd/"` | Patterns to ignore when sending processes to StackState. |
...
| clusterAgent.image.tag | string | `"2.0.6"` | Default container image tag. |
...
| clusterAgent.resources.limits.cpu | string | `"50m"` | CPU resource limits. |
| clusterAgent.resources.limits.memory | string | `"64Mi"` | Memory resource limits. |
