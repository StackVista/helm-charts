...
|-----|------|---------|-------------|
| clusterDomain | string | `"cluster.local"` | Default cluster domain to use for internal DNS. |
...
| networkPolicy.enabled | bool | `false` | Enable creating of `NetworkPolicy` object and assosicated rules for StackState. |
| networkPolicy.spec | object | `{"egress":[{}],"ingress":[{"from":[{"podSelector":{}}]}],"podSelector":{"matchLabels":{}},"policyTypes":["Ingress"]}` | `NetworkPolicy` rules for StackState. |
...
| stackstate.components.all.image.tag | string | `"sts-private-v1-16-0-charlie"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
