...
| agent.containers.agent.image.tag | string | `"2.17.1"` | Default container image tag. |
...
| clusterChecks.image.tag | string | `"2.17.1"` | Default container image tag. |
...
| stackstateAgent.affinity | object | `{}` | Affinity settings for pod assignment. |
| stackstateAgent.collection.kubernetesEvents | bool | `true` | Enable / disable the cluster agent events collection. |
| stackstateAgent.collection.kubernetesMetrics | bool | `true` | Enable / disable the cluster agent metrics collection. |
| stackstateAgent.collection.kubernetesResources.configmaps | bool | `true` | Enable / disable collection of ConfigMaps. |
| stackstateAgent.collection.kubernetesResources.cronjobs | bool | `true` | Enable / disable collection of CronJobs. |
| stackstateAgent.collection.kubernetesResources.daemonsets | bool | `true` | Enable / disable collection of DaemonSets. |
| stackstateAgent.collection.kubernetesResources.deployments | bool | `true` | Enable / disable collection of Deployments. |
| stackstateAgent.collection.kubernetesResources.endpoints | bool | `true` | Enable / disable collection of Endpoints. If endpoints are disabled then StackState won't be able to connect a Service to Pods that serving it |
| stackstateAgent.collection.kubernetesResources.ingresses | bool | `true` | Enable / disable collection of Ingresses. |
| stackstateAgent.collection.kubernetesResources.jobs | bool | `true` | Enable / disable collection of Jobs. |
| stackstateAgent.collection.kubernetesResources.namespaces | bool | `true` | Enable / disable collection of Namespaces. |
| stackstateAgent.collection.kubernetesResources.persistentvolumeclaims | bool | `true` | Enable / disable collection of PersistentVolumeClaims. Disabling these will not let StackState connect PersistentVolumes to pods they are attached to |
| stackstateAgent.collection.kubernetesResources.persistentvolumes | bool | `true` | Enable / disable collection of PersistentVolumes. |
| stackstateAgent.collection.kubernetesResources.replicasets | bool | `true` | Enable / disable collection of ReplicaSets. |
| stackstateAgent.collection.kubernetesResources.secrets | bool | `true` | Enable / disable collection of Secrets. |
| stackstateAgent.collection.kubernetesResources.statefulsets | bool | `true` | Enable / disable collection of StatefulSets. |
| stackstateAgent.collection.kubernetesTimeout | int | `10` | Default timeout (in seconds) when obtaining information from the Kubernetes API. |
| stackstateAgent.collection.kubernetesTopology | bool | `true` | Enable / disable the cluster agent topology collection. |
| stackstateAgent.config | object | `{"configMap":{"maxDataSize":null},"override":[],"topology":{"collectionInterval":90}}` |  |
| stackstateAgent.config.configMap.maxDataSize | string | `nil` | Maximum amount of characters for the data property of a ConfigMap collected by the kubernetes topology check |
| stackstateAgent.config.override | list | `[]` | A list of objects containing three keys `name`, `path` and `data`, specifying filenames at specific paths which need to be (potentially) overridden using a mounted configmap |
| stackstateAgent.config.topology.collectionInterval | int | `90` | Interval for running topology collection, in seconds |
| stackstateAgent.enabled | bool | `true` | Enable / disable the cluster agent. |
| stackstateAgent.image.pullPolicy | string | `"IfNotPresent"` | Default container image pull policy. |
| stackstateAgent.image.repository | string | `"stackstate/stackstate-stackstate-agent"` | Base container image repository. |
| stackstateAgent.image.tag | string | `"2.17.1"` | Default container image tag. |
| stackstateAgent.livenessProbe.enabled | bool | `true` | Enable use of livenessProbe check. |
| stackstateAgent.livenessProbe.failureThreshold | int | `3` | `failureThreshold` for the liveness probe. |
| stackstateAgent.livenessProbe.initialDelaySeconds | int | `15` | `initialDelaySeconds` for the liveness probe. |
| stackstateAgent.livenessProbe.periodSeconds | int | `15` | `periodSeconds` for the liveness probe. |
| stackstateAgent.livenessProbe.successThreshold | int | `1` | `successThreshold` for the liveness probe. |
| stackstateAgent.livenessProbe.timeoutSeconds | int | `5` | `timeoutSeconds` for the liveness probe. |
| stackstateAgent.logLevel | string | `"INFO"` | Logging level for stackstate-agent processes. |
| stackstateAgent.nodeSelector | object | `{}` | Node labels for pod assignment. |
| stackstateAgent.priorityClassName | string | `""` | Priority class for stackstate-agent pods. |
| stackstateAgent.readinessProbe.enabled | bool | `true` | Enable use of readinessProbe check. |
| stackstateAgent.readinessProbe.failureThreshold | int | `3` | `failureThreshold` for the readiness probe. |
| stackstateAgent.readinessProbe.initialDelaySeconds | int | `15` | `initialDelaySeconds` for the readiness probe. |
| stackstateAgent.readinessProbe.periodSeconds | int | `15` | `periodSeconds` for the readiness probe. |
| stackstateAgent.readinessProbe.successThreshold | int | `1` | `successThreshold` for the readiness probe. |
| stackstateAgent.readinessProbe.timeoutSeconds | int | `5` | `timeoutSeconds` for the readiness probe. |
| stackstateAgent.replicaCount | int | `1` | Number of replicas of the cluster agent to deploy. |
| stackstateAgent.resources.limits.cpu | string | `"400m"` | CPU resource limits. |
| stackstateAgent.resources.limits.memory | string | `"1024Mi"` | Memory resource limits. |
| stackstateAgent.resources.requests.cpu | string | `"100m"` | CPU resource requests. |
| stackstateAgent.resources.requests.memory | string | `"256Mi"` | Memory resource requests. |
| stackstateAgent.serviceaccount.annotations | object | `{}` | Annotations for the service account for the cluster agent pods |
| stackstateAgent.strategy | object | `{"type":"RollingUpdate"}` | The strategy for the Deployment object. |
| stackstateAgent.tolerations | list | `[]` | Toleration labels for pod assignment. |
