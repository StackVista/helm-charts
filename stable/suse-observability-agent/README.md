...
Current chart version is `1.6.6`
...
| checksAgent.image.tag | string | `"d5bc4a82"` | Default container image tag. |
...
| clusterAgent.image.tag | string | `"d5bc4a82"` | Default container image tag. |
...
| httpHeaderInjectorWebhook.certificatePrehook.image.tag | string | `"1.8.6-so25"` |  |
...
| logsAgent.image.tag | string | `"3.6.11-so18"` | Default container image tag. |
...
| nodeAgent.containers.agent.image.tag | string | `"d5bc4a82"` | Default container image tag. |
...
| nodeAgent.containers.processAgent.image.tag | string | `"af40af2b"` | Default process-agent container image tag. |
...
| otel.integrations.rancherAgent | bool | `false` | Enable Rancher Manager URL enrichment. When true, the collector reads the Rancher URL from a deployment in cattle-system and the cluster ID for the Harvester cluster from a secret.  Both are attached as resource attributes on all emitted log records. Defaults to false: on non-Rancher clusters the cattle-system namespace is absent so the necessary Role and RoleBinding cannot be created. |
| otel.integrations.suseAdmissionController | bool | `true` | Enable pre-configured API group filters for the SUSE Admission Controller (Kubewarden) stackpack. |
| otel.integrations.suseRuntimeEnforcer | bool | `true` | Enable pre-configured API group filters for the SUSE Runtime Enforcer stackpack. |
| otel.integrations.suseSbomScanner | bool | `false` | Enable pre-configured API group filters for the SUSE SBOM Scanner stackpack. |
| otel.integrations.suseVirtualization | bool | `true` | Enable pre-configured API group filters for the SUSE Virtualization (KubeVirt) stackpack. |
...
| otel.k8sResourceCollector.image.tag | string | `"v0.0.56-agent"` | Container image tag for 'opentelemetry-collector' containers. |
...
| otel.prometheusScraping.collector.image.tag | string | `"v0.0.56-agent"` | Container image tag for the Prometheus scraper collector. Uses the strict agent collector BOM image (the "-agent" suffixed tag). |
...
| otel.prometheusScraping.targetAllocator.image.tag | string | `"0.153.0-so14"` | SUSE Observability Target Allocator image tag (<upstream-version>-so<release-increment>). |
...
| otel.telemetryGateway.image.tag | string | `"v0.0.56-agent"` | Container image tag for the telemetry gateway. Uses the strict agent collector BOM image (the "-agent" suffixed tag). |
