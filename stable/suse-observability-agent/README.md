...
Current chart version is `1.6.8`
...
| checksAgent.image.tag | string | `"0812a1b8"` | Default container image tag. |
...
| httpHeaderInjectorWebhook.sidecarInjector.image.tag | string | `"0978b47b-1040-release"` |  |
...
| nodeAgent.containers.agent.image.tag | string | `"0812a1b8"` | Default container image tag. |
...
| otel.k8sResourceCollector.image.tag | string | `"v0.0.57-agent"` | Container image tag for 'opentelemetry-collector' containers. |
...
| otel.prometheusScraping.collector.image.tag | string | `"v0.0.57-agent"` | Container image tag for the Prometheus scraper collector. Uses the strict agent collector BOM image (the "-agent" suffixed tag). |
...
| otel.telemetryGateway.image.tag | string | `"v0.0.57-agent"` | Container image tag for the telemetry gateway. Uses the strict agent collector BOM image (the "-agent" suffixed tag). |
