...
#
images=()
while IFS='' read -r line; do images+=("$line"); done < <(helm template stackstate "$helm_chart" --repo "$helm_repository" --set "stackstate.baseUrl=http://dummy,stackstate.admin.authentication.password=dummy,stackstate.authentication.adminPassword=dummy,stackstate.license.key=dummy" | grep image: | sed -E 's/^.*image: "?([^"]*)"?.*$/\1/')
