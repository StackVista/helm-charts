...
Current chart version is `4.3.1-rc.1`
...
| https://helm.stackstate.io | anomaly-detection | 4.3.0-snapshot.165.releasing-4.3.1.8df74f12 |
...
| hbase.stackgraph.image.tag | string | `"4.0.8"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"4.3.1-rc.1"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.components.checks.resources | object | `{"limits":{"cpu":"1500m","memory":"4000Mi"},"requests":{"cpu":"1000m","memory":"4000Mi"}}` | Resource allocation for `state` pods. |
