...
Current chart version is `4.5.6-rc.2`
...
| https://helm.stackstate.io | anomaly-detection | 4.5.6-rc.2 |
...
| elasticsearch.imageTag | string | `"7.6.2-sts.20220316.0825"` | Overrides the image tag in the dependent chart, for sts 4.5.6 and later versions of 4.5 |
...
| hbase.stackgraph.image.tag | string | `"4.2.19"` | The StackGraph server version, must be compatible with the StackState version |
...
| kafka.image.tag | string | `"2.3.1-focal-20220316-r41.1"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** |
...
| stackstate.components.all.image.tag | string | `"4.5.6-rc.2"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
