...
Current chart version is `4.2.5-snapshot.1`
...
| anomaly-detection.image.tag | string | `"latest"` | the chart image tag, e.g. 4.1.3-latest tag: |
...
| hbase.stackgraph.image.tag | string | `"3.7.4"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"4.2.4"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
