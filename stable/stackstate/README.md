...
Current chart version is `5.1.13-rc.2`
...
| https://helm.stackstate.io | anomaly-detection | 5.1.13-rc.2 |
...
| hbase.stackgraph.image.tag | string | `"4.9.7"` | The StackGraph server version, must be compatible with the StackState version |
...
| kafka.image.tag | string | `"2.8.1-18f0cfea"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| stackstate.components.all.image.tag | string | `"5.1.13-rc.2"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.components.initializer.resources | object | `{"limits":{"cpu":"500m","ephemeral-storage":"1Gi","memory":"1500Mi"},"requests":{"cpu":"500m","ephemeral-storage":"1Mi","memory":"700Mi"}}` | Resource allocation for `initializer` pods. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"2.8.1-18f0cfea"` | Container image tag for kafka-topic-create containers. |
