...
Current chart version is `4.1.0-rc3`
...
| hbase.hbase.regionserver.resources.requests.cpu | string | `"2000m"` |  |
| hbase.hdfs.datanode.replicaCount | int | `3` | Number of HDFS datanode replicas. |
| hbase.hdfs.secondarynamenode.enabled | bool | `true` |  |
| hbase.stackgraph.image.tag | string | `"2.6.2"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"release-4.1.0-rc3"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
