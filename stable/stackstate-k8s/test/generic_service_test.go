...
func TestAllServicesRenderedSplit(t *testing.T) {
	...
	podsToCheck := append([]string{"api", "checks", "correlate", "initializer", "receiver", "slicing", "state", "problem-producer", "sync", "view-health", "e2es", "trace2es"})
...
func TestAllServicesRenderedServer(t *testing.T) {
	...
	resources := helmtestutil.NewKubernetesResources(t, output)
	podsToCheck := append([]string{"server", "correlate", "receiver", "e2es", "trace2es"})
