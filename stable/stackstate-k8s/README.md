...
Current chart version is `1.10.1-pre.2`
...
| https://helm.stackstate.io | anomaly-detection | 5.2.0-snapshot.121 |
...
| hbase.stackgraph.image.tag | string | `"7.1.1"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240411092708-master-91af6ab"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
