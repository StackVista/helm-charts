...
Current chart version is `1.10.1-pre.106`
...
| hbase.stackgraph.version | string | `"7.4.0"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240605073022-master-9a3674b"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240604182224-master-73c853b-selfhosted"` | Tag used for the `stackpacks` Docker image; |
