...
Current chart version is `1.10.1-pre.81`
...
| hbase.stackgraph.version | string | `"7.3.3"` | The StackGraph server version, must be compatible with the StackState version |
...
| opentelemetry-collector.image.tag | string | `"STAC-21255-components-for-databases"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240517120700-master-02fc190"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240517132459-master-a3d6435-selfhosted"` | Tag used for the `stackpacks` Docker image; |
