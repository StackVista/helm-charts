...
Current chart version is `1.10.1-pre.113`
...
| hbase.stackgraph.version | string | `"7.4.1"` | The StackGraph server version, must be compatible with the StackState version |
...
| opentelemetry-collector.image.tag | string | `"v0.0.12"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240607115946-master-77e81d5"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240606112348-master-3c54317-selfhosted"` | Tag used for the `stackpacks` Docker image; |
