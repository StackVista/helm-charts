...
Current chart version is `1.10.1-pre.64`
...
| hbase.stackgraph.version | string | `"7.3.2"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240508094620-master-fab0557"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240513140702-master-557993b-selfhosted"` | Tag used for the `stackpacks` Docker image; |
