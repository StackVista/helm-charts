...
Current chart version is `1.3.1-pre.26`
...
| file://../victoria-metrics-single/ | victoria-metrics-0(victoria-metrics-single) | 0.8.53-stackstate.4 |
| file://../victoria-metrics-single/ | victoria-metrics-1(victoria-metrics-single) | 0.8.53-stackstate.4 |
...
| hbase.stackgraph.image.tag | string | `"4.13.1"` | The StackGraph server version, must be compatible with the StackState version |
...
| kafka.image.tag | string | `"3.3.1-f2c6b50d"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.17.0-c73ad127"` | Kafka JMX exporter image tag |
...
| minio.image.tag | string | `"RELEASE.2021-02-14T04-01-33Z-709fe718"` |  |
...
| stackstate-k8s-agent.enabled | bool | `false` |  |
| stackstate-k8s-agent.stackstate.cluster.authToken | string | `nil` |  |
| stackstate-k8s-agent.stackstate.cluster.name | string | `nil` |  |
| stackstate-k8s-agent.stackstate.url | string | `"http://{{ include \"stackstate.hostname.prefix\" . }}-router:8080/receiver/stsAgent"` |  |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20231128123928-master-416565e"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.3.1-f2c6b50d"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.router.image.tag | string | `"v1.19.1-66d4d31b"` | Tag used for the `router` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.stackpacks.image | object | `{"pullPolicy":"","registry":"quay.io","repository":"stackstate/stackpacks","tag":"20231128114253-master-32e2748-selfhosted"}` | Docker image to use as source for stackpacks. |
...
| stackstate.stackpacks.image.tag | string | `"20231128114253-master-32e2748-selfhosted"` | Tag used for the `stackpacks` Docker image; |
