...
Current chart version is `1.10.1-pre.166`
...
| hbase.stackgraph.version | string | `"7.5.1"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240621210043-master-ffa068a"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240620110345-master-aef4f3b-selfhosted"` | Tag used for the `stackpacks` Docker image; |
