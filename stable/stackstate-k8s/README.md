...
Current chart version is `1.9.3-pre.14`
...
| https://helm.stackstate.io | anomaly-detection | 5.2.0-snapshot.111 |
...
| hbase.stackgraph.image.tag | string | `"6.0.0"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240409070711-master-a3818ad"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240408142840-master-4e0eed0-selfhosted"` | Tag used for the `stackpacks` Docker image; |
