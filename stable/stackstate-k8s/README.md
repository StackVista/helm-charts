...
Current chart version is `1.10.1-pre.234`
...
| hbase.stackgraph.version | string | `"7.6.6"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240718165243-master-aeb7185"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.tag | string | `"20240715081237-master-00ac651-selfhosted"` | Tag used for the `stackpacks` Docker image; |
