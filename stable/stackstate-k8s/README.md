...
Current chart version is `1.7.1-pre.83`
...
| opentelemetry-collector.image.tag | string | `"v0.0.3"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240219080643-master-ab6eb9c"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
