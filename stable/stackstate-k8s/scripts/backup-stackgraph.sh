...
if [ "${BACKUP_STACKGRAPH_ARCHIVE_SPLIT_SIZE:-0}" == "0" ]; then
    uploadFileToS3 "${TMP_DIR}/${BACKUP_FILE}" "s3://${BACKUP_STACKGRAPH_BUCKET_NAME}/${BACKUP_FILE}" "http://${MINIO_ENDPOINT}"
else
    ...
    do
        uploadFileToS3 "${file}" "s3://${BACKUP_STACKGRAPH_BUCKET_NAME}/$(basename "${file}")" "http://${MINIO_ENDPOINT}"
    done
fi
