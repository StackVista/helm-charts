...
// TestElasticsearchGlobalCommonLabels tests that global.commonLabels are applied to StatefulSet and Pod templates
func TestElasticsearchGlobalCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/global-common-labels.yaml")
...
// TestElasticsearchCommonLabels tests that commonLabels are applied to StatefulSet and Pod templates
func TestElasticsearchCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/common-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestElasticsearchPodLabels tests that podLabels are applied only to Pod templates, not StatefulSet
func TestElasticsearchPodLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/pod-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestElasticsearchAllLabelsCombined tests the precedence and combination of all label types
func TestElasticsearchAllLabelsCombined(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestElasticsearchDefaultLabels tests that default chart labels are always present
func TestElasticsearchDefaultLabels(t *testing.T) {
...
// TestElasticsearchLabelPrecedenceDoesNotOverrideDefaults tests that custom labels don't override required defaults
func TestElasticsearchLabelPrecedenceDoesNotOverrideDefaults(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
	...
	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/name", "StatefulSet should have app.kubernetes.io/name label")
	assert.Equal(t, "elasticsearch", statefulset.Labels["app.kubernetes.io/name"], "app.kubernetes.io/name should be 'elasticsearch'")

	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/instance", "StatefulSet should have app.kubernetes.io/instance label")
	assert.Equal(t, "elasticsearch", statefulset.Labels["app.kubernetes.io/instance"], "app.kubernetes.io/instance should be 'elasticsearch' (release name)")

	// Check Pod template as well
	podLabels := statefulset.Spec.Template.Labels
	assert.Contains(t, podLabels, "app.kubernetes.io/name", "Pod template should have app.kubernetes.io/name label")
	assert.Equal(t, "elasticsearch", podLabels["app.kubernetes.io/name"], "Pod template app.kubernetes.io/name should be 'elasticsearch'")
}

// TestElasticsearchGlobalCommonLabelsWithPrometheusExporter tests that global.commonLabels are applied to both Elasticsearch StatefulSet and prometheus-elasticsearch-exporter Deployment
func TestElasticsearchGlobalCommonLabelsWithPrometheusExporter(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "elasticsearch", "values/global-common-labels-with-prometheus-exporter.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)

	// Should have one StatefulSet (elasticsearch) and one Deployment (prometheus-elasticsearch-exporter)
	assert.Len(t, resources.Statefulsets, 1, "Should have exactly one StatefulSet")
	assert.Len(t, resources.Deployments, 1, "Should have exactly one Deployment")

	// Check Elasticsearch StatefulSet labels
	var statefulset appsv1.StatefulSet
	for _, ss := range resources.Statefulsets {
		statefulset = ss
		break
	...
	assert.Contains(t, statefulset.Labels, "global.label1", "Elasticsearch StatefulSet should have global.label1")
	assert.Equal(t, "global-value1", statefulset.Labels["global.label1"], "Elasticsearch StatefulSet global.label1 should have correct value")
	assert.Contains(t, statefulset.Labels, "global.label2", "Elasticsearch StatefulSet should have global.label2")
	assert.Equal(t, "global-value2", statefulset.Labels["global.label2"], "Elasticsearch StatefulSet global.label2 should have correct value")

	// Check prometheus-elasticsearch-exporter Deployment labels
	...
	assert.Contains(t, deployment.Labels, "global.label1", "Prometheus Exporter Deployment should have global.label1")
	assert.Equal(t, "global-value1", deployment.Labels["global.label1"], "Prometheus Exporter Deployment global.label1 should have correct value")
	assert.Contains(t, deployment.Labels, "global.label2", "Prometheus Exporter Deployment should have global.label2")
	assert.Equal(t, "global-value2", deployment.Labels["global.label2"], "Prometheus Exporter Deployment global.label2 should have correct value")

	// Check Pod template labels for both resources
	esPodLabels := statefulset.Spec.Template.Labels
	assert.Contains(t, esPodLabels, "global.label1", "Elasticsearch Pod template should have global.label1")
	assert.Equal(t, "global-value1", esPodLabels["global.label1"], "Elasticsearch Pod template global.label1 should have correct value")

	exporterPodLabels := deployment.Spec.Template.Labels
	assert.Contains(t, exporterPodLabels, "global.label1", "Prometheus Exporter Pod template should have global.label1")
	assert.Equal(t, "global-value1", exporterPodLabels["global.label1"], "Prometheus Exporter Pod template global.label1 should have correct value")
	assert.Contains(t, exporterPodLabels, "global.label2", "Prometheus Exporter Pod template should have global.label2")
	assert.Equal(t, "global-value2", exporterPodLabels["global.label2"], "Prometheus Exporter Pod template global.label2 should have correct value")
}
