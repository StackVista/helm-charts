...
Current chart version is `2.3.3-pre.14`
...
| role-k8s-authz.authprovider | string | `""` | Prefix added to OIDC groups by a proxy like Rancher (e.g. 'keycloakoidc_group://') |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250507075118-master-1482326"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250506095736-master-ab127da"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
