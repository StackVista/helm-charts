...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260721134206-master-6d0eb4d"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.images[0].version | string | `"20260721110025-master-c70a2f6"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
