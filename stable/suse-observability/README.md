...
Current chart version is `2.10.3-pre.69`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.214"` | the chart image tag, e.g. 4.1.3-latest |
...
| elasticsearch.imageTag | string | `"8.19.16-so7"` | Elasticsearch image tag. Updated by updatecli. |
...
| hbase.hdfs.version | string | `"3.5.0-so8"` | Full HDFS image tag. Updated by updatecli. |
...
| kubernetes-rbac-agent.containers.rbacAgent.image.tag | string | `"e1054bcf-1016-release"` |  |
...
| opentelemetry-collector.image.tag | string | `"v0.0.51"` | Container image tag for 'opentelemetry-collector' containers. |
...
| s3proxy.image.tag | string | `"3.2.0-so6"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260722044123-41739e51"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260722100141-master-5f48566"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-so15"` | Container image tag for container-tools containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260722042851-6608c5f9"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-so15"` | Container image tag for router mode containers. |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-so15"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so11"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-so15"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so11"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"1.144.0-so10"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"3.9.5-so16"` | ZooKeeper image tag |
