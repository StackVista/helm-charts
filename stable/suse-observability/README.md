...
Current chart version is `2.1.2-pre.25`
...
| hbase.stackgraph.version | string | `"7.8.0"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20241112120254-master-1df0f19"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.email.smtpSslEnable | bool | `true` | Enable SSL for SMTP configuration for StackState |
...
| stackstate.stackpacks.image.version | string | `"20241112130618-master-3aa249e"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
