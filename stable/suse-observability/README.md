...
Current chart version is `2.10.3-pre.5`
...
| clickhouse.backup.image.tag | string | `"2.7.2-so2"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"26.5.1-so2"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"8.19.16-so3"` | Elasticsearch image tag. Updated by updatecli. |
...
| hbase.hdfs.version | string | `"3.5.0-so5"` | Full HDFS image tag. Updated by updatecli. |
| hbase.stackgraph.version | string | `"8.1.6"` |  |
...
| kafka.image.tag | string | `"3.9.2-so9"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so5"` | Kafka JMX exporter image tag |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260630131427-master-109dc4a"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"26.5.1-so2"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.2-so9"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.vmagent.image.tag | string | `"1.144.0-so6"` |  |
...
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so7"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"1.144.0-so6"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so7"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"1.144.0-so6"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"1.144.0-so6"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"3.9.5-so11"` | ZooKeeper image tag |
