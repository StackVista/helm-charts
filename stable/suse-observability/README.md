...
Current chart version is `2.10.2-pre.49`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.206"` | the chart image tag, e.g. 4.1.3-latest |
...
| clickhouse.backup.image.tag | string | `"2.7.0-so2"` | Container image tag for 'clickhouse' backup containers. |
...
| elasticsearch.imageTag | string | `"8.19.15-so3"` | Elasticsearch image tag. Updated by updatecli. |
| elasticsearch.prometheus-elasticsearch-exporter.image.tag | string | `"1.10.0-so4"` | Elasticsearch Prometheus exporter image tag. Updated by updatecli. |
...
| global.wait.image.tag | string | `"1.0.12-so4"` | Container image tag for wait containers. |
...
| kafka.image.tag | string | `"3.9.2-so3"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so3"` | Kafka JMX exporter image tag |
...
| s3proxy.image.tag | string | `"3.1.0-so3"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260608051049-d6114ed5"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-so5"` | Container image tag for container-tools containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.2-so3"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260608045915-c816bce4"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.nginxPrometheusExporter.image.tag | string | `"1.5.1-so4"` | Container image tag for nginx-prometheus-exporter containers. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-so5"` | Container image tag for router mode containers. |
...
| stackstate.components.vmagent.image.tag | string | `"1.144.0-so2"` |  |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-so5"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so3"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"1.144.0-so2"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-so5"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so3"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"1.144.0-so2"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"1.144.0-so2"` | Container image tag for 'vmrestore' containers. |
