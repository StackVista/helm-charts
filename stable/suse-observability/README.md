...
Current chart version is `2.10.3-pre.61`
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260720113751-master-c20475e"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.images[0].version | string | `"20260720112434-master-43dbe43"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
