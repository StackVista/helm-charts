...
Current chart version is `2.7.1-pre.102`
...
| file://../opentelemetry-collector | opentelemetry-collector | 0.108.0-stackstate.20 |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260226162511-master-cf74430"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20260217135717-master-6ce0195"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
