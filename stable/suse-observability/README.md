...
Current chart version is `2.10.4-pre.40`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.218"` | the chart image tag, e.g. 4.1.3-latest |
...
| backup.stackGraph.v2.tempData.accessModes[0] | string | `"ReadWriteOnce"` |  |
| backup.stackGraph.v2.tempData.size | string | `"20Gi"` |  |
| backup.stackGraph.v2.tempData.storageClass | string | `nil` |  |
...
| clickhouse.backup.image.tag | string | `"2.7.2-so10"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"26.5.1-so8"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"8.19.16-so12"` | Elasticsearch image tag. Updated by updatecli. |
| elasticsearch.prometheus-elasticsearch-exporter.image.tag | string | `"1.10.0-so14"` | Elasticsearch Prometheus exporter image tag. Updated by updatecli. |
...
| global.features | object | `{"experimentalStackpacks":false,"experimentalSuppressions":false}` | Feature switches for SUSE Observability. |
...
| global.features.experimentalSuppressions | bool | `false` | Enable Suppressions to signal to all components that they should support the Suppressions feature. This is a preproduction feature, usage may break your entire installation with upcoming releases. No backwards compatibility is guaranteed. |
...
| global.wait.image.tag | string | `"1.0.12-so9"` | Container image tag for wait containers. |
...
| hbase.hdfs.version | string | `"3.5.0-so13"` | Full HDFS image tag. Updated by updatecli. |
| hbase.stackgraph.version | string | `"8.3.5"` |  |
...
| kafka.image.tag | string | `"3.9.2-so17"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so11"` | Kafka JMX exporter image tag |
...
| kafkaup-operator.image.tag | string | `"0.0.14"` |  |
...
| opentelemetry-collector.image.tag | string | `"v0.0.54"` | Container image tag for 'opentelemetry-collector' containers. |
...
| s3proxy.image.tag | string | `"3.2.0-so10"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260819035201-f3be0255"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260820095920-master-2337bbb"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"26.5.1-so8"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-so19"` | Container image tag for container-tools containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.2-so17"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260819033531-3d67dda0"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.nginxPrometheusExporter.image.tag | string | `"1.5.1-so13"` | Container image tag for nginx-prometheus-exporter containers. |
...
| stackstate.components.router.image.tag | string | `"1.38.3-so2"` | Tag used for the `router` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-so19"` | Container image tag for router mode containers. |
...
| stackstate.components.vmagent.image.tag | string | `"1.144.0-so12"` |  |
...
| stackstate.components.workloadObserver.image.tag | string | `"64188fd0-1030-release"` | Tag used for the `workloadObserver` component Docker image.. |
...
| stackstate.stackpacks.images[0].version | string | `"20260819105948-master-9675369"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
...
| stackstate.stackpacks.images[1].tag | string | `"20260818153229-main-1a59b6e"` | Tag used for the `stackpacks` Docker image |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-so19"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so15"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"1.144.0-so12"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-so19"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so15"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"1.144.0-so12"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"1.144.0-so14"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"3.9.5-so20"` | ZooKeeper image tag |
