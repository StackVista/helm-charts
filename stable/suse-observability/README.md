...
Current chart version is `2.11.3-pre.32`
...
even with the default release name. Update external manifests and automation that reference
the previous UI or replication-checker resource names.
Allow for a UI interruption and restart any active replication check after the upgrade.
When the vmagent ConfigMap name changes, its existing StatefulSet rolls the pod to reference
the new name. With the default single vmagent replica, including HA sizing profiles, this
temporarily interrupts metrics ingestion. The persistent queue retains the same PVC.
...
| global.features | object | `{"experimentalOtelLogs":false,"experimentalSilencing":false}` | Feature switches for SUSE Observability. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260923200914-master-95b5666"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.images[0].version | string | `"20260923125936-master-5c2fee9"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
...
| stackstate.stackpacks.images[1].tag | string | `"20260923110120-main-db19448"` | Tag used for the `stackpacks` Docker image |
