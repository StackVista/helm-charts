...
Current chart version is `2.10.2-pre.60`
...
| hbase.stackgraph.version | string | `"8.1.1"` |  |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260610075952-master-e3f9c39"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20260610110724-master-4814eb5"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
