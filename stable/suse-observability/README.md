...
Current chart version is `2.10.4-pre.63`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.219"` | the chart image tag, e.g. 4.1.3-latest |
...
| clickhouse.backup.image.tag | string | `"2.7.2-so11"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"26.5.1-so9"` | Container image tag for 'clickhouse' containers. |
...
| global.features | object | `{"experimentalSilencing":false}` | Feature switches for SUSE Observability. |
...
| hbase.hdfs.version | string | `"3.5.0-so14"` | Full HDFS image tag. Updated by updatecli. |
| hbase.stackgraph.version | string | `"8.3.11"` |  |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so12"` | Kafka JMX exporter image tag |
...
| kubernetes-rbac-agent.containers.rbacAgent.image.tag | string | `"23f4b80a-1032-release"` |  |
...
| opentelemetry-collector.image.tag | string | `"v0.0.56"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260829034217-f3be0255"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260903140504-master-b9a98ae"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"26.5.1-so9"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-so20"` | Container image tag for container-tools containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260829033100-3d67dda0"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-so20"` | Container image tag for router mode containers. |
...
| stackstate.components.workloadObserver.image.tag | string | `"64188fd0-1031-release"` | Tag used for the `workloadObserver` component Docker image.. |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-so20"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so16"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-so20"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so16"` | VM backup image tag. Updated by updatecli. |
...
| zookeeper.image.tag | string | `"3.9.5-so21"` | ZooKeeper image tag |
