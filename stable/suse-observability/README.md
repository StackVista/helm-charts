...
Current chart version is `2.3.8-pre.56`
...
| stackstate.allowedOrigins | list | `[]` |  |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250819095847-master-0ae2df8"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250818074653-master-2730757"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
