...
Current chart version is `2.9.1-pre.30`
...
| opentelemetry-collector.image.tag | string | `"v0.0.29"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260403101004-master-add5a73"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
