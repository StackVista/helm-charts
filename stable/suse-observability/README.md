...
Current chart version is `2.6.3-pre.33`
...
| hbase.stackgraph.version | string | `"7.12.5"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20251117083932-master-4ddf98b"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20251105153923-master-a224663"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
