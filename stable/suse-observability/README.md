...
Current chart version is `2.3.2-pre.32`
...
| https://helm.stackstate.io | kubernetes-rbac-agent | 0.0.13 |
...
| opentelemetry-collector.image.tag | string | `"v0.0.18"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250328174216-master-c001646"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250401084442-master-bcd414a"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
