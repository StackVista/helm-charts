...
Current chart version is `2.3.1-pre.63`
...
| https://helm.stackstate.io | kubernetes-rbac-agent | 0.0.12 |
...
| hbase.stackgraph.version | string | `"7.9.4"` | The StackGraph server version, must be compatible with the StackState version |
...
| kubernetes-rbac-agent.containers.rbacAgent.podAnnotations | object | `{"ad.stackstate.com/rbac-agent.check_names":"[\"openmetrics\"]","ad.stackstate.com/rbac-agent.init_configs":"[{}]","ad.stackstate.com/rbac-agent.instances":"[ { \"prometheus_url\": \"http://%%host%%:8080/metrics\", \"namespace\": \"stackstate\", metrics: [\"*\"] } ]"}` | Additional annotations on the pod |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250314100845-master-8891f9f"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250305092809-master-bc7ef66"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
