...
Current chart version is `2.6.4-pre.4`
...
| hbase.stackgraph.version | string | `"7.13.3"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.stackpacks.image.version | string | `"20251121095203-master-3673f57"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
