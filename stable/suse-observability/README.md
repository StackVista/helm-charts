...
Current chart version is `2.3.4-pre.48`
...
| hbase.stackgraph.version | string | `"7.9.7"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250602125627-master-81d69a1"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250530074753-master-3ddb220"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
