...
Current chart version is `2.8.2-pre.17`
...
| clickhouse.backup.image.tag | string | `"2.6.39-a7171eea-145-release"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"25.9.5-c65c568c-release-161"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"8.19.4-c4527168-release-208"` | Elasticsearch image tag. Updated by updatecli. |
...
| elasticsearch.prometheus-elasticsearch-exporter.image.tag | string | `"v1.8.0-bf725da8-release-159"` | Elasticsearch Prometheus exporter image tag. Updated by updatecli. |
...
| global.suseObservability | object | `{"adminPassword":"","adminPasswordBcrypt":"","affinity":{"nodeAffinity":null,"podAffinity":null,"podAntiAffinity":{"requiredDuringSchedulingIgnoredDuringExecution":true,"topologyKey":"kubernetes.io/hostname"}},"baseUrl":"","license":"","modes":["Observability"],"pullSecret":{"password":"","username":""},"receiverApiKey":"","sizing":{"profile":""}}` | Simplified configuration section that allows users to specify high-level settings. When any values in this section are configured (license, baseUrl, sizing.profile, etc.), the chart will automatically use this configuration instead of the legacy stackstate.* values. This provides a single-chart installation experience without needing the separate suse-observability-values chart. NOTE: This section works in conjunction with existing global settings (imageRegistry, receiverApiKey, imagePullSecrets). IMPORTANT: When using this section, also set global.imageRegistry to "registry.rancher.com" for SUSE Observability images. |
...
| global.suseObservability.modes | list | `["Observability"]` | Product modes. Default is ["Observability"]. Only "Observability" is currently supported. |
...
| global.wait.image.tag | string | `"1.0.12-6dabb0fe-release-185"` | Container image tag for wait containers. |
...
| hbase.hdfs.version | string | `"java21-8-aef270ee-release-361"` | HDFS image build version (e.g. java21-8-27156f06-353). Derived from hadoop docker tag with semver prefix stripped. Updated by updatecli. |
...
| kafka.image.tag | string | `"3.9.1-2f5d8989-286-release"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-58a72255-287-release"` | Kafka JMX exporter image tag |
...
| minio.image.tag | string | `"RELEASE.2025-11-19T12-41-50Z-e064c44f-185-release"` | MinIO image tag. Updated by updatecli. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260320122854-master-26e52c0"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"25.9.5-c65c568c-release-161"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.3-610"` | Container image tag for container-tools containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.1-2f5d8989-286-release"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260320080627-affe318b"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.nginxPrometheusExporter.image.tag | string | `"1.5.1-fdbee6c2-95-release"` | Container image tag for nginx-prometheus-exporter containers. |
...
| stackstate.components.router.image.tag | string | `"v1.31.1-38312d95-release-166"` | Tag used for the `router` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.3-610"` | Container image tag for router mode containers. |
...
| stackstate.components.vmagent.image.tag | string | `"v1.109.0-cbe73f5f-release-139"` |  |
...
| stackstate.components.workloadObserver.image.tag | string | `"ab88e7da-100-release"` | Tag used for the `workloadObserver` component Docker image.. |
...
| stackstate.stackpacks.image.version | string | `"20260319154911-master-d135a6d"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
...
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"v1.109.0-3cb9d6ad-release-55"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"v1.109.0-9bebf8bd-release-145"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"v1.109.0-3cb9d6ad-release-55"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"v1.109.0-9bebf8bd-release-145"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"v1.109.0-d1e2ca0c-release-42"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"3.9.3-352e04a1-291-release"` | ZooKeeper image tag |
