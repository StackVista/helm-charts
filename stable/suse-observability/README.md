...
Current chart version is `2.10.3-pre.74`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.215"` | the chart image tag, e.g. 4.1.3-latest |
...
| clickhouse.backup.image.tag | string | `"2.7.2-so7"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"26.5.1-so5"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"8.19.16-so8"` | Elasticsearch image tag. Updated by updatecli. |
| elasticsearch.prometheus-elasticsearch-exporter.image.tag | string | `"1.10.0-so11"` | Elasticsearch Prometheus exporter image tag. Updated by updatecli. |
...
| global.wait.image.tag | string | `"1.0.12-so8"` | Container image tag for wait containers. |
...
| hbase.hdfs.version | string | `"3.5.0-so10"` | Full HDFS image tag. Updated by updatecli. |
| hbase.stackgraph.version | string | `"8.1.11"` |  |
...
| kafka.image.tag | string | `"3.9.2-so14"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so8"` | Kafka JMX exporter image tag |
...
| kafkaup-operator.image.tag | string | `"0.0.13"` |  |
...
| opentelemetry-collector.image.tag | string | `"v0.0.52"` | Container image tag for 'opentelemetry-collector' containers. |
...
| s3proxy.image.tag | string | `"3.2.0-so7"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260723070847-ff93fb37"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260723102451-master-30c73aa"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"26.5.1-so5"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-so16"` | Container image tag for container-tools containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.2-so14"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260723042754-6608c5f9"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.nginxPrometheusExporter.image.tag | string | `"1.5.1-so11"` | Container image tag for nginx-prometheus-exporter containers. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-so16"` | Container image tag for router mode containers. |
...
| stackstate.components.vmagent.image.tag | string | `"1.144.0-so10"` |  |
...
| stackstate.components.workloadObserver.image.tag | string | `"40b2a4b2-1022-release"` | Tag used for the `workloadObserver` component Docker image.. |
...
| stackstate.stackpacks.images[0].version | string | `"20260721142410-master-d600e27"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
...
| stackstate.stackpacks.images[1].tag | string | `"20260721144515-main-838a6b4"` | Tag used for the `stackpacks` Docker image |
...
| stackstate.stackpacks.images[2].tag | string | `"20260720124210-main-223827d"` | Tag used for the `stackpacks` Docker image |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-so16"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"1.144.0-so12"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"1.144.0-so10"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-so16"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"1.144.0-so12"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"1.144.0-so10"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"1.144.0-so11"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"3.9.5-so17"` | ZooKeeper image tag |
