...
Current chart version is `2.7.1-pre.107`
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260303083826-master-c30187f"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20260227100514-master-916d9dc"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
