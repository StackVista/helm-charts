...
Current chart version is `2.3.7-hotfix.1`
...
| hbase.stackgraph.version | string | `"7.10.5"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250728075040-version-2.3.6-bb2abf0"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.k8sAuthorization.enabled | bool | `false` |  |
...
| stackstate.stackpacks.image.version | string | `"20250710080552-master-98ca665"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
