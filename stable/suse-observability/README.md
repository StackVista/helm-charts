...
Current chart version is `2.6.4-pre.55`
...
| hbase.stackgraph.version | string | `"7.13.12"` | The StackGraph server version, must be compatible with the StackState version |
...
| opentelemetry-collector.image.tag | string | `"v0.0.24"` | Container image tag for 'opentelemetry-collector' containers. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20251218155552-master-115d3d7"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20251218112130-master-cea96c7"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
