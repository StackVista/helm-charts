...
Current chart version is `2.3.6-pre.27`
...
| hbase.stackgraph.version | string | `"7.9.10"` | The StackGraph server version, must be compatible with the StackState version |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250702145610-master-4fd329f"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250625122411-master-3c0f9fe"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
