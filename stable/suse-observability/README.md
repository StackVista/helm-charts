...
Current chart version is `2.7.1-pre.22`
...
| file://../opentelemetry-collector | opentelemetry-collector | 0.108.0-stackstate.17 |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260127094631-master-64eeab9"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20260115100547-master-e222137"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
