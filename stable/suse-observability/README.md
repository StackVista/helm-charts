...
Current chart version is `2.5.0-pre.11`
...
| file://../victoria-metrics-single/ | victoria-metrics-1(victoria-metrics-single) | 0.8.53-stackstate.23 |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250910084040-master-27b7a9c"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.components.router.mode.image.tag | string | `"1.7.0"` | Container image tag for router mode containers. |
...
| stackstate.stackpacks.image.version | string | `"20250903124600-master-a104702"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
