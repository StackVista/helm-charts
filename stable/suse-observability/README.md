...
Current chart version is `2.10.2-pre.113`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.210"` | the chart image tag, e.g. 4.1.3-latest |
...
| clickhouse.backup.image.tag | string | `"2.7.2-so1"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"25.12.8-so1"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"8.19.16-so2"` | Elasticsearch image tag. Updated by updatecli. |
...
| hbase.hdfs.version | string | `"3.5.0-so3"` | Full HDFS image tag. Updated by updatecli. |
| hbase.stackgraph.version | string | `"8.1.5"` |  |
...
| kafka.image.tag | string | `"3.9.2-so7"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-so4"` | Kafka JMX exporter image tag |
...
| opentelemetry-collector.image.tag | string | `"v0.0.42"` | Container image tag for 'opentelemetry-collector' containers. |
...
| s3proxy.image.tag | string | `"3.1.0-so7"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260626050102-99f7b839"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260626055504-master-6c14f85"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"25.12.8-so1"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"3.9.2-so7"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260626044853-5721dae6"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.stackpacks.image.version | string | `"20260624161259-master-d683c31"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
