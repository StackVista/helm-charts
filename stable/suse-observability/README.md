...
Current chart version is `2.10.1-pre.38`
...
| anomaly-detection.image.tag | string | `"5.2.0-snapshot.202"` | the chart image tag, e.g. 4.1.3-latest |
...
| clickhouse.backup.image.tag | string | `"2.6.43-566580fb-main-104"` | Container image tag for 'clickhouse' backup containers. |
...
| clickhouse.image.tag | string | `"rebuilt_zero_cve_25.9.6-566580fb-main-18"` | Container image tag for 'clickhouse' containers. |
...
| elasticsearch.imageTag | string | `"rebuilt_zero_cve_8.19.15-566580fb-main-23"` | Elasticsearch image tag. Updated by updatecli. |
...
| hbase.hdfs.version | string | `"java21-9-3c1f24de-release-396"` | HDFS image build version (e.g. java21-8-27156f06-353). Derived from hadoop docker tag with semver prefix stripped. Updated by updatecli. |
...
| kafka.image.tag | string | `"rebuilt_zero_cve_3.9.2-566580fb-main-35"` | Kafka image tag. **Since StackState relies on this specific version, it's advised NOT to change this.** When changing this version, be sure to change the pod annotation stackstate.com/kafkaup-operator.kafka_version aswell, in order for the kafkaup operator to upgrade the inter broker protocol version |
...
| kafka.metrics.jmx.image.tag | string | `"0.20.0-566580fb-main-12"` | Kafka JMX exporter image tag |
...
| s3proxy.image.tag | string | `"3.1.0-566580fb-main-66"` | Image tag for S3Proxy |
...
| stackstate.components.aiAssistant.image.tag | string | `"20260520045822-45510412"` | Tag used for the `ai-assistant` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260521133636-master-43230ef"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.components.clickhouseCleanup.image.tag | string | `"rebuilt_zero_cve_25.9.6-566580fb-main-18"` | Container image tag for 'clickhouseCleanup' containers. |
...
| stackstate.components.containerTools.image.tag | string | `"1.8.6-566580fb-main-107"` | Container image tag for container-tools containers. |
...
| stackstate.components.kafkaTopicCreate.image.tag | string | `"rebuilt_zero_cve_3.9.2-566580fb-main-35"` | Container image tag for kafka-topic-create containers. |
...
| stackstate.components.mcp.image.tag | string | `"20260520044726-ce870052"` | Tag used for the `mcp` component Docker image; this will override `stackstate.components.all.image.tag` on a per-service basis. |
...
| stackstate.components.router.mode.image.tag | string | `"1.8.6-566580fb-main-107"` | Container image tag for router mode containers. |
...
| stackstate.components.vmagent.image.tag | string | `"v1.143.0-566580fb-main-74"` |  |
...
| stackstate.components.workloadObserver.image.tag | string | `"5fe52727-200-release"` | Tag used for the `workloadObserver` component Docker image.. |
...
| victoria-metrics-0.backup.setupCron.image.tag | string | `"1.8.6-566580fb-main-107"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-0.backup.vmbackup.image.tag | string | `"v1.143.0-566580fb-main-74"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-0.server.image.tag | string | `"v1.143.0-566580fb-main-74"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics-1.backup.setupCron.image.tag | string | `"1.8.6-566580fb-main-107"` | Container-tools image for cron setup. Updated by updatecli. |
| victoria-metrics-1.backup.vmbackup.image.tag | string | `"v1.143.0-566580fb-main-74"` | VM backup image tag. Updated by updatecli. |
...
| victoria-metrics-1.server.image.tag | string | `"v1.143.0-566580fb-main-74"` | Victoria Metrics server image tag. Updated by updatecli. |
...
| victoria-metrics.restore.image.tag | string | `"v1.143.0-566580fb-main-74"` | Container image tag for 'vmrestore' containers. |
...
| zookeeper.image.tag | string | `"rebuilt_zero_cve_3.9.5-566580fb-main-21"` | ZooKeeper image tag |
