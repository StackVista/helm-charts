...
Current chart version is `2.5.1-pre.24`
...
| https://helm.stackstate.io | kubernetes-rbac-agent | 0.0.18 |
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20250918115450-master-e4f4a70"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20250918090144-master-79d12f1"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
