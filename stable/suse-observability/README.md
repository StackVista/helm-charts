...
Current chart version is `2.0.0-pre.11`
...
| stackstate.components.all.image.tag | string | `"6.0.0-snapshot.20240902073847-master-400bca3"` | The default tag used for all stateless components of StackState; invividual service `tag`s can be overriden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20240902100403-master-6777867"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
