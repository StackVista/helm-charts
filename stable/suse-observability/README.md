...
Current chart version is `2.9.1-pre.179`
...
| stackstate.components.all.image.tag | string | `"7.0.0-snapshot.20260505100525-master-5a93157"` | The default tag used for all stateless components of StackState; individual service `tag`s can be overridden (see below). |
...
| stackstate.stackpacks.image.version | string | `"20260501132313-master-3b506fb"` | Version used for the `stackpacks` Docker image, the tag is build from the version and the stackstate edition + deployment mode |
