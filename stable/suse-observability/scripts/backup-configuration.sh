...
current_files=$(sts-toolbox aws s3 ls --endpoint "${S3_ENDPOINT}" --region us-east-1 --bucket "${S3_BUCKET_SETTINGS}" --prefix "" 2>/dev/null | grep -E '\.sty$' | awk '{print $NF}' | sort || true)
...
if [ "$current_count" -ge "$BACKUP_CONFIGURATION_MAX_LOCAL_FILES" ]; then
  ...
  echo "$current_files" | head -n "$files_to_remove" | while read -r file; do
    ...
    sts-toolbox aws s3 rm --endpoint "${S3_ENDPOINT}" --region us-east-1 "s3://${S3_BUCKET_SETTINGS}/${file}" || true
  done
fi

echo "=== Listing contents of settings bucket \"${S3_BUCKET_SETTINGS}\"..."
sts-toolbox aws s3 ls --endpoint "${S3_ENDPOINT}" --region us-east-1 --bucket "${S3_BUCKET_SETTINGS}" --prefix "" 2>/dev/null | awk '{print $NF}' || true
...
sts-toolbox aws s3 cp --endpoint "${S3_ENDPOINT}" --region us-east-1 "${BACKUP_FILE_WITH_PATH}" "s3://${S3_BUCKET_SETTINGS}/${BACKUP_FILE}"
...
if [ "$BACKUP_CONFIGURATION_UPLOAD_REMOTE" == "true" ]; then
    ...
    sts-toolbox aws s3 cp --endpoint "${S3_ENDPOINT}" --region us-east-1 "${BACKUP_FILE_WITH_PATH}" "s3://${BACKUP_CONFIGURATION_BUCKET_NAME}/${BACKUP_CONFIGURATION_S3_PREFIX}${BACKUP_FILE}"
