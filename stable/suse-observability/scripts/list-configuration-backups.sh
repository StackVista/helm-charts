...
sts-toolbox aws s3 ls --endpoint "${S3_ENDPOINT}" --region us-east-1 --bucket "${S3_BUCKET_SETTINGS}" --prefix "" 2>/dev/null || echo "(no backups found)"
echo "==="
...
if [ "$BACKUP_CONFIGURATION_UPLOAD_REMOTE" == "true" ]; then
  ...
  sts-toolbox aws s3 ls --endpoint "${S3_ENDPOINT}" --region us-east-1 --bucket "${BACKUP_CONFIGURATION_BUCKET_NAME}" --prefix "${BACKUP_CONFIGURATION_S3_PREFIX}" 2>/dev/null || echo "(no backups found)"
  echo "==="
fi
