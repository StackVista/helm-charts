...
function listImages() {
  ...
  helm_values_base="ai.assistant.enabled=true,anomaly-detection.enabled=true,global.backup.enabled=true,backup.storage.backend.pvc.enabled=true,backup.storage.credentials.accessKey=ABCDEFGH,backup.storage.credentials.secretKey=ABCDEFGHABCDEFGH,stackstate.baseUrl=http://dummy.stackstate.io,stackstate.admin.authentication.password=dummy,stackstate.authentication.adminPassword=dummy,stackstate.license.key=dummy,global.receiverApiKey=dummy,clickhouse.enabled=true,opentelemetry.enabled=true,stackstate.k8sAuthorization.enabled=true"
