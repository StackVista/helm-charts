...
local update_docker_images = {
  ...
  ignore_mcp_version_update_trigger: job('UPDATE_MCP_DOCKER_VERSION', ['echo "MCP image updates are handled by the scheduled updatecli Docker image pipeline."']),
  ignore_ai_assistant_version_update_trigger: job('UPDATE_AI_ASSISTANT_DOCKER_VERSION', ['echo "AI Assistant image updates are handled by the scheduled updatecli Docker image pipeline."']),
