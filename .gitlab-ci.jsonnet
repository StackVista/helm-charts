...
local validate_and_push_jobs = {
  ...
  validate_stackstate_chart: {
    ...
    script: [
      'ct list-changed --config test/ct.yaml',
      'ct lint --debug --validate-maintainers=false --charts stackstate --config test/ct.yaml',
      '.gitlab/validate_kubeval.sh',
    ],
    stage: 'validate',
  } + skip_when_sg_upgrade,
...
'on_success') + {
  ...
  { before_script: ['.gitlab/bump_sts_chart_master_version.sh stackstate-internal'] }
  else {}
  ))
  for chart in charts
};
...
'manual') + {
    ...
    if chart == 'stackstate' then
  { before_script: ['.gitlab/bump_sts_chart_master_version.sh stackstate'] }
  else {}
  ))
  for chart in charts
};
