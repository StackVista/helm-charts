...
// TestZookeeperGlobalCommonLabels tests that global.commonLabels are applied to StatefulSet and Pod templates
func TestZookeeperGlobalCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "zookeeper", "values/global-common-labels.yaml")
...
// TestZookeeperCommonLabels tests that commonLabels are applied to StatefulSet and Pod templates
func TestZookeeperCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "zookeeper", "values/common-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestZookeeperPodLabels tests that podLabels are applied only to Pod templates, not StatefulSet
func TestZookeeperPodLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "zookeeper", "values/pod-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestZookeeperAllLabelsCombined tests the precedence and combination of all label types
func TestZookeeperAllLabelsCombined(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "zookeeper", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestZookeeperDefaultLabels tests that default chart labels are always present
func TestZookeeperDefaultLabels(t *testing.T) {
...
// TestZookeeperLabelPrecedenceDoesNotOverrideDefaults tests that custom labels don't override required defaults
func TestZookeeperLabelPrecedenceDoesNotOverrideDefaults(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "zookeeper", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
	...
	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/name", "StatefulSet should have app.kubernetes.io/name label")
	assert.Equal(t, "zookeeper", statefulset.Labels["app.kubernetes.io/name"], "app.kubernetes.io/name should be 'zookeeper'")

	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/instance", "StatefulSet should have app.kubernetes.io/instance label")
	assert.Equal(t, "zookeeper", statefulset.Labels["app.kubernetes.io/instance"], "app.kubernetes.io/instance should be 'zookeeper' (release name)")

	// Check Pod template as well
	podLabels := statefulset.Spec.Template.Labels
	assert.Contains(t, podLabels, "app.kubernetes.io/name", "Pod template should have app.kubernetes.io/name label")
	assert.Equal(t, "zookeeper", podLabels["app.kubernetes.io/name"], "Pod template app.kubernetes.io/name should be 'zookeeper'")
}
