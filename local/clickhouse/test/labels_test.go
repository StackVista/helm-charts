...
// TestClickhouseGlobalCommonLabels tests that global.commonLabels are applied to StatefulSet and Pod templates
func TestClickhouseGlobalCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "clickhouse", "values/global-common-labels.yaml")
...
// TestClickhouseCommonLabels tests that commonLabels are applied to StatefulSet and Pod templates
func TestClickhouseCommonLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "clickhouse", "values/common-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestClickhousePodLabels tests that podLabels are applied only to Pod templates, not StatefulSet
func TestClickhousePodLabels(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "clickhouse", "values/pod-labels.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestClickhouseAllLabelsCombined tests the precedence and combination of all label types
func TestClickhouseAllLabelsCombined(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "clickhouse", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
...
// TestClickhouseDefaultLabels tests that default chart labels are always present
func TestClickhouseDefaultLabels(t *testing.T) {
...
// TestClickhouseLabelPrecedenceDoesNotOverrideDefaults tests that custom labels don't override required defaults
func TestClickhouseLabelPrecedenceDoesNotOverrideDefaults(t *testing.T) {
	output := helmtestutil.RenderHelmTemplate(t, "clickhouse", "values/all-labels-combined.yaml")
	resources := helmtestutil.NewKubernetesResources(t, output)
	...
	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/name", "StatefulSet should have app.kubernetes.io/name label")
	assert.Equal(t, "clickhouse", statefulset.Labels["app.kubernetes.io/name"], "app.kubernetes.io/name should be 'clickhouse'")

	assert.Contains(t, statefulset.Labels, "app.kubernetes.io/instance", "StatefulSet should have app.kubernetes.io/instance label")
	assert.Equal(t, "clickhouse", statefulset.Labels["app.kubernetes.io/instance"], "app.kubernetes.io/instance should be 'clickhouse' (release name)")

	// Check Pod template as well
	podLabels := statefulset.Spec.Template.Labels
	assert.Contains(t, podLabels, "app.kubernetes.io/name", "Pod template should have app.kubernetes.io/name label")
	assert.Equal(t, "clickhouse", podLabels["app.kubernetes.io/name"], "Pod template app.kubernetes.io/name should be 'clickhouse'")
}
