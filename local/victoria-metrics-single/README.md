...
| server.image.tag | string | `"v1.109.0-accea47f"` | Image tag |
...
| server.podAnnotations | object | `{"ad.stackstate.com/vmbackup.check_names":"[\"openmetrics\"]","ad.stackstate.com/vmbackup.init_configs":"[{}]","ad.stackstate.com/vmbackup.instances":"[ { \"prometheus_url\": \"http://%%host%%:9746/metrics\", \"namespace\": \"stackstate\", \"metrics\": [\"supercronic_*\"] } ]","ad.stackstate.com/{{ include \"victoria-metrics.name\" . }}-{{ .Values.server.name }}.check_names":"[\"openmetrics\"]","ad.stackstate.com/{{ include \"victoria-metrics.name\" . }}-{{ .Values.server.name }}.init_configs":"[{}]","ad.stackstate.com/{{ include \"victoria-metrics.name\" . }}-{{ .Values.server.name }}.instances":"[ { \"prometheus_url\": \"http://%%host%%:8428/metrics\", \"namespace\": \"stackstate\", \"metrics\": [\"vm*\", \"go*\", \"process_open_fds\", \"process_max_fds\", \"process_cpu_cores_available\"] } ]"}` | Pod's annotations (Datadog autodiscovery for VM server + vmbackup metrics). Autodiscovery matches the key against the container name, so the server key must follow this instance's alias. |
...
| server.vmbackupmanager.image.tag | string | `"v1.93.7-enterprise"` | vmbackupmanager image tag |
