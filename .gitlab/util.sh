...
function push_changes_skip_ci() {
  for branch in $BRANCHES; do
    if [[ "${PROMOTION_DRY_RUN}" == 'no' ]]; then
      ...
      git push "https://gitlab-ci-token:${CI_JOB_TOKEN:?}@gitlab.com/stackvista/devops/helm-charts.git" HEAD:"${branch}" -o ci.skip # I have to use option `ci.skip' instead of a commit message otherwise the pipeline isn't trigger for a tag creation
    else
...
function push_tag_skip_ci() {
  ...
  git push "https://gitlab-ci-token:${CI_JOB_TOKEN:?}@gitlab.com/stackvista/devops/helm-charts.git" "${tag}" -o ci.skip
}
...
function push_changes() {
  for branch in $BRANCHES; do
    if [[ "${PROMOTION_DRY_RUN}" == 'no' ]]; then
      echo "Pushing changes"
      git pull --rebase origin "${branch}"
      git push "https://gitlab-ci-token:${CI_JOB_TOKEN:?}@gitlab.com/stackvista/devops/helm-charts.git" HEAD:"${branch}"
    else
      echo "Not pushing changes, set PROMOTION_DRY_RUN='no' to commit changes"
    fi
  done
