...
class BumpChartVersion:
    ...
    def update_chart_version(self, chart_name: str, new_version: str) -> None:
        ...
        if self.dry_run:
            self.log_info(f"[DRY-RUN] Would update {chart_yaml_path} to version {new_version}")
            return
    ...
    def update_dependency_version(
        ...
        self.log_debug(f"Updating dependency {dep_name} to version {new_version} in {chart_yaml_path}")
    ...
    def build_dependency_graph(self) -> None:
        ...
        self.log_debug(f"Dependency graph built. Found reverse dependencies for {len(self.reverse_deps)} charts.")
    ...
    def update_chart_dependencies(self, chart_name: str) -> None:
        ...
        for dep in dependencies:
            ...
            if dep_name in self.bumped_versions and repository.startswith('file://'):
                ...
                if self.dry_run:
                    self.log_info(f"[DRY-RUN] Would update dependency {dep_name} to {new_version} in {chart_name}")
                else:
                    self.log_info(f"Updating dependency {dep_name} to {new_version} in {chart_name}")
    ...
    def regenerate_chart_lock(self, chart_name: str) -> None:
        ...
        try:
            ...
            if result.returncode != 0:
                self.log_warn(f"Failed to update dependencies for {chart_name}: {result.stderr}")
            elif self.verbose:
                for line in result.stdout.strip().split('\n'):
                    if line:
                        self.log_debug(f"  helm: {line}")
    ...
    def run(self, target_chart: str, bump_type: str = 'patch') -> None:
        ...
        if not target_chart_yaml.exists():
            self.log_error(f"Chart not found: {target_chart} (expected at {target_chart_yaml})")
            sys.exit(1)
        ...
        if not dependents:
            ...
            for dependent in dependents:
                ...
                self.log_info(f"Processing dependent: {dependent} ({dep_current_version} -> {dep_new_version})")
...
but will NOT have their own version bumped.
    ...
    if not Path(args.charts_dir).is_dir():
        print(f"{Colors.RED}[ERROR]{Colors.NC} Charts directory not found: {args.charts_dir}", file=sys.stderr)
        sys.exit(1)
